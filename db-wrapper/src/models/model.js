  ...
  return new Promise((resolve, reject) => {
    getFightersByName(fighterName)
      .then(response => {
        ...
        const query = {
          text:
            "SELECT COUNT(*) FROM (SELECT fighter_1 fighter_id from matches UNION ALL SELECT fighter_2 fighter_id from matches) AS full_list where fighter_id = $1",
          values: [fighterID]
        };
...
function getWinsAgainst(fighter1ID, fighter2ID) {
  return new Promise((resolve, reject) => {
    const query = {
      text:
        "SELECT count(*) FROM matches WHERE winner = $1 AND (fighter_1 = $2 OR fighter_2 = $2)",
    ...
    getFightersByName(fighterName)
      .then(rows => {
        if (rows.length == 0) {
          const query = {
            text:
              "INSERT INTO fighters(name, tier, asof) VALUES ($1, $2, CURRENT_DATE) RETURNING *",
...
function insertMatch(fighter1, fighter2, winnerID, format, dateID) {
  const query = {
    text:
      "INSERT INTO matches(date, fighter_1, fighter_2, bet_1, bet_2, winner, format) VALUES($1, $2, $3, $4, $5, $6, $7) RETURNING *",
    values: [
      dateID,
      fighter1.id,
      fighter2.id,
