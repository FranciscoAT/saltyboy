...
def db_get_match_count(
    ...
    **kwargs,
    ...
    if bet_red__gte:
        where_stmts.append("bet_red__gte >= %(bet_red__gte)s")
        query_obj["bet_red__gte"] = bet_red__gte
    if bet_red__lt:
        where_stmts.append("bet_red__lt < %(bet_red__lt)s")
        query_obj["bet_red__lt"] = bet_red__lt
    if bet_blue__gte:
        where_stmts.append("bet_blue__gte >= %(bet_blue__gte)s")
        query_obj["bet_blue__gte"] = bet_blue__gte
    if bet_blue__lt:
        where_stmts.append("bet_blue__lt < %(bet_blue__lt)s")
        query_obj["bet_blue__lt"] = bet_blue__lt
    if streak_red__gte:
        where_stmts.append("streak_red__gte >= %(streak_red__gte)s")
        query_obj["streak_red__gte"] = streak_red__gte
    if streak_red__lt:
        where_stmts.append("streak_red__lt < %(streak_red__lt)s")
        query_obj["streak_red__lt"] = streak_red__lt
    if streak_blue__gte:
        where_stmts.append("streak_blue__gte >= %(streak_blue__gte)s")
        query_obj["streak_blue__gte"] = streak_blue__gte
    if streak_blue__lt:
        where_stmts.append("streak_blue__lt < %(streak_blue__lt)s")
